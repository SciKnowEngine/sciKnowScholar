 ...
 * 
 */
public class FragmentCollectionReader extends CitationListCollectionReader {
	
	private static Logger logger = Logger.getLogger(FragmentCollectionReader.class);
		
	public static final String FRG_TYPE = ConfigurationParameterFactory
			.createConfigurationParameterName(FragmentCollectionReader.class,
					"frgType");
	@ConfigurationParameter(mandatory = true, description = "The type of fragments extracted")
	...
	private static String sql = 
			...
			"inner join FTDFragment as f on (f.ftd_id = d.vpdmfId) " + 
			"inner join FTDFragmentBlock as b on (b.fragment_id = f.vpdmfId) " + 
			" WHERE l.vpdmfId=";
