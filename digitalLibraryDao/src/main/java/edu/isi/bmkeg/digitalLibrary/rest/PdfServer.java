...
public class PdfServer {
	...
	public ResponseEntity<byte []> byPdfParameter(@RequestParam("swfFile") String fileName) throws Exception {
		...
		ArticleCitation_qo lc = new ArticleCitation_qo();
		qFtd.setCitation(lc);
		lc.setPmid(String.valueOf(vpdmfId));
...
//		    responseHeaders.set("Content-Disposition", "attachment; filename=\"" +  fileName + '\"');
			...
			// TODO: Need to load SWF from disk, not from DB
	...
	public ResponseEntity<String> byXmlParameter(@RequestParam("xmlFile") String fileName) throws Exception {
		...
		if( l.size() == 1 ) {
			...
			// TODO: Need to load XML from disk, not from DB
	...
	@RequestMapping(value="/load", 
			method=RequestMethod.GET, 
			params="nxml")
	public ResponseEntity<String> byPmcXmlParameter(@RequestParam("nxml") String fileName) throws Exception {
		...
		Pattern patt = Pattern.compile("(\\d+)\\.nxml");
		Matcher m = patt.matcher(fileName);
		...
		if( l.size() == 1 ) {
			vpdmfId = l.get(0).getVpdmfId();
			FTD ftd = this.ftdDao.findArticleDocumentById(vpdmfId);
			String path = ftd.getName().replaceAll("\\.pdf", ".nxml");
			File xml = new File( this.ftdDao.getCoreDao().getWorkingDirectory() + "/" + path);
			String xmlContents = FileUtils.readFileToString(xml, "UTF-8");
			response = new ResponseEntity<String> (
					xmlContents, responseHeaders, HttpStatus.OK
					);
    	} 
	...
	@RequestMapping(value="/load", 
			method=RequestMethod.GET, 
			params="txt")
	public ResponseEntity<String> byTxtParameter(@RequestParam("txt") String fileName) throws Exception {
