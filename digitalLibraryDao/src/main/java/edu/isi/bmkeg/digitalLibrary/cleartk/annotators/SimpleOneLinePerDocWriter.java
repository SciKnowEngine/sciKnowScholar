...
import edu.isi.bmkeg.digitalLibrary.uimaTypes.citations.LiteratureCitation;
...
public class SimpleOneLinePerDocWriter extends JCasAnnotator_ImplBase {
	...
	public void process(JCas jCas) throws AnalysisEngineProcessException {
		...
		LiteratureCitation lc = JCasUtil.selectSingle(jCas, LiteratureCitation.class);
				
		File outFile = new File(baseData.getPath() + "/" + c.getName() + ".txt" ); 
		
		try {
			
			PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter(outFile, true)));

			out.print(ftd.getVpdmfId() + "	");
