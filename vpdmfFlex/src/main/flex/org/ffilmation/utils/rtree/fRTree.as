...
package org.ffilmation.utils.rtree  {
	...
	import flash.utils.*;
		...
		public class fRTree {
			...
			private var nearestIdDistances:Array = new Array
				
			public static var debugFlag:Boolean = false;
			...
			private function recursiveNearest(p:Array, n:fRTreeNode, nearestDistance:Number):Number {
				for (var i:int = 0; i < n.entryCount; i++) {
					...
					//~Bug fixin'~~~~~~~~~~~~~
					var cube:fCube = n.entries[i];
					var tempDistance:Number = cube.distanceToPoint(p)
					//~~~~~~~~~~~~~~~~~~~~~~~~
					if (n.isLeaf()) { // for leaves, the distance is an actual nearest distance 
						if (tempDistance < nearestDistance) {
							nearestDistance = tempDistance
							this.nearestIds = [];
							this.nearestIdDistances = [];
						}
						if (tempDistance <= nearestDistance) {
							this.nearestIds[this.nearestIds.length] = n.ids[i];
							this.nearestIdDistances[this.nearestIdDistances.length] = tempDistance;
							if( debugFlag ) 
								trace( "point: " + p.toString() + ", cube: " 
									+ cube.toString() + ", d: " + tempDistance );
						}     
